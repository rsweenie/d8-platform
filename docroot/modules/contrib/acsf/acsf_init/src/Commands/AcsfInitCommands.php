...
/**
 * This namespace choice is off but it is necessary. All the commands in this
 * file are executed with the --include flag, and without reusing this namespace
 * the commands would not be found.
 */
...
class AcsfInitCommands extends DrushCommands {
   ...
   */
  public function init($options = ['skip-default-settings' => FALSE]) {
   ...
   *   If something wrong with the current codebase.
   */
  public function initVerify($options = ['skip-default-settings' => NULL]) {
  ...
  /**
   * Connect a site to a factory by setting up the right variables in the
   * database.
   *
   ...
   */
  public function connectFactory($options = [
    ...
    if ($site_admin_mail_account && $site_admin_mail_account->id() > 1) {
      throw new AcsfException(dt('Unable to sync the admin account, the email address @mail is already used by the user account @uid.', array(
        '@mail' => $site_admin_mail,
    ...
    foreach ($site_owner_roles as $owner_role) {
      if (Role::load($owner_role)) {
        ...
        $this->logger()->warning(dt('The role @role does not exist; not adding it to the site owner.', array('@role' => $owner_role)));
      }
    }
  ...
  /**
   * Uninstalls components for this Drupal repository to be compatible with
   * Acquia Site Factory.
   *
  ...
  public function uninstall() {
    ...
    foreach ($this->getRequiredFiles($repo_root) as $location) {
      ...
      if (isset($location['source']) && file_exists($dest)) {
        $confirm = $this->io()->confirm(dt('Delete !file?', array('!file' => $dest)));
        if ($confirm === FALSE) {
          continue;
        }
        if (unlink($dest)) {
