...
class AcsfDuplicationCommands extends DrushCommands {
   ...
   */
  public function duplicationScrubBatch($site_name, $standard_domain, $options = ['exact-copy' => NULL, 'avoid-oom' => NULL, 'batch' => NULL, 'batch-comment' => NULL, 'batch-node' => NULL, 'batch-user' => NULL, 'retain-content' => NULL, 'retain-users' => NULL]) {
    if (empty($site_name)) {
  ...
  public function duplicationScrubProgress() {
    ...
    $empty_event = AcsfEvent::create('site_duplication_scrub', array(), $this->output());
    $data = array();
