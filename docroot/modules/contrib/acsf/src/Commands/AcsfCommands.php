...
class AcsfCommands extends DrushCommands {
   ...
   */
  public function siteSync($options = ['data' => NULL]) {
    $site = AcsfSite::load();
    $data = $options['data'];
  ...
  public function siteScrub() {
    ...
    if (!empty($preserved_roles)) {
      $this->output()->writeln(dt('Preserving roles: @rids', array('@rids' => implode(', ', $preserved_roles))));
    }
    ...
    $this->output()->writeln(dt('Preserving users: @uids', array('@uids' => implode(', ', $preserved_users))));
